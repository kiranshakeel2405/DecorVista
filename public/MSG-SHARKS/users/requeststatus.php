...
$userId = $_SESSION['userId'];
$query = "SELECT emergency_requests.*,drivers.name FROM emergency_requests LEFT JOIN drivers ON emergency_requests.driver_id = drivers.driver_id WHERE emergency_requests.user_id = '$userId' AND emergency_requests.is_deleted = '0'";
                                ...
                                if (count($allRequests) > 0) {
                                    foreach ($allRequests as $request) {
                                        ...
                                        <tr>
                                            ...
                                            <td><a href="<?= (isset($request['ambulance_id']) && !empty($request['ambulance_id']))?'./ambulances.php?id='.$request['ambulance_id']:'' ?>"><?= (isset($request['ambulance_id']) && !empty($request['ambulance_id']))?'View Details':'Not assigned yet' ?></a></td>
                                            <td><?= $request['type'] ?></td>
                                            <td>
                                            ...
                                            <td><a
                                                    href="<?= (isset($request['name'])) ? './driverdetail.php?id='.$request['driver_id'].'' : "" ?>"><?= (isset($request['name'])) ? $request['name'] : "Not Assigned Yet" ?></a>
                                            </td>
                                            <td>
                                                <?php 
                                                    if(in_array($request['status'],['dispatched','on-route','arrived'])){
                                                ?>
                                                <a href="./tracklocation.php?id=<?= $request['ambulance_id']?>&request_id=<?= $request['request_id'] ?>"><i
                                                        class="bx bx-map bx-md"></i></a>

                                                <?php
                                                    }else if($request['status'] == 'pending'){
                                                        echo 'No action to show';
                                                    }else{
                                                    ?>
                                                    <a href="./feedback.php?id=<?= $request['request_id'] ?>"><i class="bx bx-message-square-dots bx-md"></i></a>
                                                    <?php } ?>
