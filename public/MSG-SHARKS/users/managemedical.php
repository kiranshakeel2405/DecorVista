...
$query = "SELECT medical_history,allergies,emergency_contact_phone FROM users WHERE user_id = '$userId'";
$medicalRecord = $db->fetchSingle($query);
$medicalhistory = $medicalRecord['medical_history'];
$allergies = $medicalRecord['allergies'];
$emergencyContact = $medicalRecord['emergency_contact_phone'];
?>
<!doctype html>
...
<body>
                            ...
                            <div class="col-xxl">
                                <div class="card mb-6">
                                    <div class="card-header d-flex align-items-center justify-content-between">
                                        <h5 class="mb-0">UPDATE YOUR MEDICAL HISTORY</h5> <small class="text-muted float-end">Save
                                            Life</small>
                                    </div>
                                    <div class="card-body">
                                        <form id="MedicalUpdateForm">
                                            <div class="row mb-6">
                                                <label class="col-sm-2 col-form-label" for="basic-default-name">Medical
                                                    History</label>
                                                <div class="col-sm-10">
                                                <textarea id="medicalhistory" class="form-control" placeholder="Enter your medical history" aria-label="Hi, Do you have a moment to talk Joe?" aria-describedby="basic-icon-default-message2" value="<?= (isset($medicalhistory)?$medicalhistory:'') ?>"><?= (isset($medicalhistory)?$medicalhistory:'') ?></textarea>
                                                    <span class="error-message" id="medicalhistory-error"></span>
                                                    <input hidden id="userId" type="text" value="<?= $userId ?>">
                                                </div>
                                            </div>
                                            <div class="row mb-6">
                                                <label class="col-sm-2 col-form-label"
                                                    for="basic-default-company">Allergies</label>
                                                <div class="col-sm-10">
                                                <textarea id="allergies" class="form-control" placeholder="Enter allergies" aria-label="Hi, Do you have a moment to talk Joe?" aria-describedby="basic-icon-default-message2" value="<?= (isset($allergies)?$allergies:'') ?>"><?= (isset($allergies)?$allergies:'') ?></textarea>
                                                    <span class="error-message" id="allergies-error"></span>
                                                </div>
                                            </div>
                                            <div class="row mb-6">
                                                <label class="col-sm-2 col-form-label"
                                                    for="basic-default-company">Emergency Contact</label>
                                                <div class="col-sm-10">
                                                    <input type="text" class="form-control" id="emergencycontact"
                                                        placeholder="Enter Emergency Contact" value="<?= (isset($emergencyContact)?$emergencyContact:'') ?>">
                                                    <span class="error-message" id="emergencycontact-error"></span>
                                                </div>
                                            </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#MedicalUpdateForm').submit(function (event) {
                event.preventDefault(); // Prevent default form submission

                var isValid = true;

                // Validate each field on form submission
                validateField('#medicalhistory', function (addr) { return addr.trim() !== ""; }, 'Medical History cannot be empty.');
                ...
                if ($('.invalid').length === 0) {
                    ...
                    let data = {
                        ...
                        userId: $('#userId').val(),
                        requestType:"updateMedicalHistory"
                    }
            ...
            $('#medicalhistory,#allergies,#emergencycontact').on('input focusout', function () {
                var fieldId = $(this).attr('id');

                // Validate the field individually
                if (fieldId === 'medicalhistory') {
                    validateField(this, function (addr) { return addr.trim() !== ""; }, 'Medical History cannot be empty.');
                } else if (fieldId === 'allergies') {
                    validateField(this, function (addr) { return addr.trim() !== ""; }, 'Allergies cannot be empty.');
                } else if (fieldId === 'emergencycontact') {
                    validateField(this, isValidPhone, 'Enter a valid emergency contact phone number (10-15 digits).');
                }
            });
        });
